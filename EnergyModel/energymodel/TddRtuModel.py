...
class TddRtuModel():
    ...
    def __vent_fan_speed(self, rtu, occ = True):
        if(occ):
    ...
    def __calculate_load_profile(self, rtu, occ = True):
        #easy reference to load_profile object
            if (occ):
                cd = rtu.hourly_data.occ_load_profile
            else:
                cd = rtu.hourly_data.unocc_load_profile
            ...
            eflh_c_eff = eflh_c / refrig_eff_pct_at_oa_t
            ...
            #vent run hours
            fan_vent_run_hours = cd.vent_hrs
            print("Occ " + str(occ) + " total hours: " + str(cd.htg_hrs + cd.clg_hrs + cd.vent_hrs))
            ...
            fan_avg_vent_speed = self.__vent_fan_speed(rtu, occ)
            ...
            fan_vent_kwh = fan_vent_run_hours * fan_avg_vent_kw_per_ton * rtu.tons
            fan_clg_kwh = fan_clg_run_hours * fan_avg_clg_kw_per_ton * rtu.tons
            fan_htg_kwh = fan_htg_run_hours * fan_avg_htg_kw_per_ton * rtu.tons
            ...
            refrig_kwh = eflh_c_eff * refrig_kw_per_ton_at_oa_t * rtu.tons

            #totalize kwh from refrig, fan, and heating
            total_kwh = fan_kwh + refrig_kwh

            rtu.fan_vent_kwh_yearly = rtu.fan_vent_kwh_yearly + fan_vent_kwh
            rtu.fan_clg_kwh_yearly = rtu.fan_clg_kwh_yearly + fan_clg_kwh
            rtu.fan_htg_kwh_yearly = rtu.fan_htg_kwh_yearly + fan_htg_kwh
            rtu.fan_kwh_yearly = rtu.fan_kwh_yearly + fan_kwh
            rtu.refrg_kwh_yearly = rtu.refrg_kwh_yearly + refrig_kwh
            rtu.kwh_hvac_yearly = rtu.kwh_hvac_yearly + total_kwh
    ...
    def calculate(self, rtu):
        ...
        if ((isinstance(rtu, Cdu.Cdu) or isinstance(rtu, Pkg.Pkg)) and rtu.tons > 0):
            occ_kwh = self.__calculate_load_profile(rtu, occ = True)
            unocc_kwh = self.__calculate_load_profile(rtu, occ = False)
